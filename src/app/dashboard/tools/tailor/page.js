...
export default function ResumeTailor() {
  ...
  const {
    ...
    onComplete: () => toast.success('Tailoring Done!', 'Your resume has been optimized for this JD.')
  });
        ...
        <div className="space-y-6">
          ...
          <Card className="bg-white border-4 border-brutal-black shadow-[4px_4px_0_rgba(0,0,0,1)]">
            <CardContent className="p-6">
              ...
              <Button 
                variant="brutal" 
                className="w-full text-xl py-6 bg-brutal-blue text-black mt-4"
          ...
          {(status === JOB_STATUS.COMPLETED || historyResult) && displayResult && (
            <>
              ...
              <ResultActions 
                ...
                className="mb-4"
              />
              <Card className="bg-white border-4 border-brutal-black shadow-[4px_4px_0_rgba(0,0,0,1)] animate-in fade-in zoom-in duration-300">
              <CardContent className="p-6">
           ...
           {(status === JOB_STATUS.COMPLETED || historyResult) && displayResult && (
             <RegenerateBlock 
               ...
               onRegenerate={(newModelId) => {
                 ...
                 setHistoryResult(null);
                 // We reuse the selectedResume and jobDescription from the current state.
                 // Ideally, we'd read them from historyResult if we loaded from history, but 
                 // the user's current selections in the UI are what we should branch off.
                 // Let's ensure we use the active inputs:
