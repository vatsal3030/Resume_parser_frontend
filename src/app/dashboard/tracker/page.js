...
const COL_COLORS = {
 'SAVED': 'bg-(--canvas)',
 'APPLIED': 'bg-(--primary)',
 'INTERVIEWING': 'bg-(--accent-amber)',
 'OFFER': 'bg-(--success)',
 'REJECTED': 'bg-(--primary-active)'
};
...
export default function ApplicationTracker() {
 ...
 return (
 <div className="mt-3 space-y-2 border-t border-dashed border-(--hairline) pt-3">
 {totalCheck > 0 && (
 ...
 <div className="w-16 bg-gray-200 h-2 border border-(--hairline) rounded-xl overflow-hidden">
 <div 
 className="bg-(--accent-amber) h-full border-r border-(--hairline)" 
 ...
 {pendingRem > 0 && (
 <div className="flex items-center gap-1.5 text-xs font-semibold text-red-600 bg-red-50 border border-red-500 p-1 px-1.5 rounded-xl shadow-sm">
 ...
 <div>
 <h1 className="text-4xl font-mediumer">Job Application Tracker</h1>
 <p className="text-sm text-(--muted) mt-1">
 Interactive Drag-and-Drop Board & Application Timeline Logs
 </p>
 </div>
 <Button variant="default" onClick={() => setShowCreateModal(true)} className="text-sm font-medium">
 ...
 {COLUMNS.map(col => (
 <div key={col} className="min-w-[240px] md:min-w-[260px] lg:min-w-0 flex-1 rounded-2xl border border-(--hairline) flex flex-col snap-center bg-(--surface-card)/40 shadow-sm overflow-hidden">
 <div className="p-4 border-b border-(--hairline-soft) bg-(--surface-soft)/50 flex justify-between items-center">
 <h2 className="text-sm font-serif font-medium text-(--ink) uppercase tracking-wider">{col}</h2>
 <span className="text-xs font-medium bg-(--surface-soft) text-(--muted) px-2.5 py-0.5 rounded-full border border-(--hairline-soft)">—</span>
 </div>
 <div className="p-4 flex-1 space-y-4 bg-transparent min-h-[550px] animate-pulse">
 {[1, 2, col === 'SAVED' ? 3 : null].filter(Boolean).map(i => (
 <div key={i} className="bg-(--surface-card) border border-(--hairline) rounded-xl p-4 space-y-3 shadow-xs">
 ...
 <div className="h-2.5 bg-(--surface-soft) rounded w-12" />
 </div>
 </div>
 ))}
 </div>
 ...
 <div 
 key={col}
 className="min-w-[240px] md:min-w-[260px] lg:min-w-0 flex-1 rounded-2xl border border-(--hairline) flex flex-col snap-center bg-(--surface-card)/40 shadow-sm overflow-hidden"
 onDragOver={onDragOver}
 onDrop={(e) => onDrop(e, col)}
 >
 <div className="p-4 border-b border-(--hairline-soft) bg-(--surface-soft)/50 flex justify-between items-center">
 <h2 className="text-sm font-serif font-medium text-(--ink) uppercase tracking-wider">{col}</h2>
 <span className="text-xs font-medium bg-(--surface-soft) text-(--muted) px-2.5 py-0.5 rounded-full border border-(--hairline-soft)">
 {columnApps.length}
 </span>
 </div>

 <div className="p-4 flex-1 overflow-y-auto space-y-4 bg-transparent min-h-[550px] max-h-[700px]">
 {columnApps.length === 0 ? (
 <div className="h-full flex items-center justify-center text-center p-8 border border-dashed border-gray-400 text-gray-500 font-bold text-sm">
 ...
 className="bg-(--surface-card) rounded-xl border border-(--hairline) p-4 cursor-grab active:cursor-grabbing hover:border-(--primary)/50 hover:shadow-md transition-all relative group shadow-xs"
 >
 <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity">
 <button 
 onClick={(e) => handleDelete(app.id, e)} 
 className="text-red-400 hover:text-red-300 bg-(--surface-soft) rounded-lg border border-(--hairline-soft) p-1 transition-all"
 >
 <Trash2 className="w-3.5 h-3.5" />
 </button>
 </div>
 
 <h3 className="font-serif text-base text-(--ink) leading-tight mb-1 pr-6 group-hover:text-(--primary) transition-colors">
 {app.role}
 </h3>
 <p className="text-xs text-(--muted) font-medium mb-2">{app.company}</p>
 
 <div className="flex flex-wrap gap-2 mb-2">
 {app.salary && (
 <span className="inline-block bg-(--success) text-black text-xs font-semibold px-2 py-0.5 border border-(--hairline)">
 {app.salary}
 </span>
 )}
 </div>
 ...
 className="inline-flex items-center text-xs font-semibold text-blue-600 hover:underline gap-0.5 mt-1"
 >
 ...
 <div className="bg-(--surface-card) rounded-2xl border border-(--hairline) shadow-2xl max-w-md w-full p-6 relative animate-in fade-in zoom-in-95 duration-150">
 <button 
 onClick={() => setShowCreateModal(false)}
 className="absolute top-4 right-4 text-black hover:bg-gray-100 border border-(--hairline) p-1 shadow-sm active:shadow-none"
 >
 <X className="w-4 h-4" />
 </button>
 
 <h2 className="text-2xl font-medium mb-4 border-b border-(--hairline) pb-2">New Job Tracking</h2>
 ...
 <input 
 required 
 className="w-full rounded-xl border border-(--hairline) bg-(--surface-soft) p-2.5 text-sm text-(--ink) focus:border-(--primary) outline-none transition-colors" 
 ...
 <div>
 <label className="block font-semibold text-sm mb-1">Role / Title *</label>
 <input 
 required 
 className="w-full rounded-xl border border-(--hairline) bg-(--surface-soft) p-2.5 text-sm text-(--ink) focus:border-(--primary) outline-none transition-colors" 
 ...
 <div>
 <label className="block font-semibold text-sm mb-1">Job Description URL</label>
 <input 
 type="url" 
 className="w-full rounded-xl border border-(--hairline) bg-(--surface-soft) p-2.5 text-sm text-(--ink) focus:border-(--primary) outline-none transition-colors" 
 ...
 <div>
 <label className="block font-semibold text-sm mb-1">Salary Range</label>
 <input 
 className="w-full rounded-xl border border-(--hairline) bg-(--surface-soft) p-2.5 text-sm text-(--ink) focus:border-(--primary) outline-none transition-colors" 
 ...
 <Button type="button" variant="white" onClick={() => setShowCreateModal(false)} className="flex-1 font-bold border border-(--hairline)">
 Cancel
 </Button>
 <Button type="submit" variant="default" className="flex-1 bg-(--primary) font-medium text-black border border-(--hairline) shadow-sm">
 Create
 </Button>
 </div>
 </form>
 ...
 <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
 <div className="bg-(--surface-card) rounded-2xl border border-(--hairline) shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col relative animate-in fade-in zoom-in-95 duration-150">
 
 {/* Modal Header */}
 <div className="p-6 border-b border-(--hairline-soft) bg-(--surface-soft)/50 flex justify-between items-start">
 <div>
 <span className="inline-block bg-black text-white text-xs font-medium px-2 py-0.5 border border-(--hairline) mb-1">
 ...
 className="text-black hover:bg-gray-100 border border-(--hairline) p-1.5 shadow-sm active:shadow-none"
 >
 <X className="w-5 h-5" />
 </button>
 </div>
 ...
 <div className="w-full md:w-60 bg-(--surface-soft)/30 border-r-0 md:border-r border-b md:border-b-0 border-(--hairline-soft) flex md:flex-col overflow-x-auto md:overflow-x-visible shrink-0 p-4 gap-1.5">
 <button 
 onClick={() => setModalTab('details')}
 className={`w-full text-left p-3 font-semibold text-sm border border-(--hairline) shadow-sm active:shadow-none flex items-center gap-2 transition-all shrink-0 md:shrink ${
 modalTab === 'details' ? 'bg-(--surface-soft) text-(--ink) border-(--primary) font-medium shadow-xs' : 'bg-transparent text-(--muted) hover:text-(--ink) hover:bg-(--surface-soft) border-transparent'
 }`}
 >
 ...
 className={`w-full text-left p-3 font-semibold text-sm border border-(--hairline) shadow-sm active:shadow-none flex items-center justify-between transition-all shrink-0 md:shrink ${
 modalTab === 'checklist' ? 'bg-(--surface-soft) text-(--ink) border-(--primary) font-medium shadow-xs' : 'bg-transparent text-(--muted) hover:text-(--ink) hover:bg-(--surface-soft) border-transparent'
 }`}
 >
 ...
 className={`w-full text-left p-3 font-semibold text-sm border border-(--hairline) shadow-sm active:shadow-none flex items-center justify-between transition-all shrink-0 md:shrink ${
 modalTab === 'reminders' ? 'bg-(--surface-soft) text-(--ink) border-(--primary) font-medium shadow-xs' : 'bg-transparent text-(--muted) hover:text-(--ink) hover:bg-(--surface-soft) border-transparent'
 }`}
 >
 <span className="flex items-center gap-2">
 ...
 <span className="text-xs bg-red-600 text-white px-1.5 font-semibold animate-pulse">
 ...
 className={`w-full text-left p-3 font-semibold text-sm border border-(--hairline) shadow-sm active:shadow-none flex items-center gap-2 transition-all shrink-0 md:shrink ${
 modalTab === 'timeline' ? 'bg-(--success) translate-x-0.5 translate-y-0.5 shadow-none' : 'bg-white hover:bg-green-50'
 }`}
 >
 ...
 <div className="bg-yellow-50 border border-(--hairline) p-4 shadow-sm flex justify-between items-center flex-wrap gap-2">
 <div className="flex items-center gap-2">
 <Info className="w-5 h-5 text-black shrink-0" />
 <span className="font-extrabold text-sm">Update Current Stage:</span>
 </div>
 <select 
 value={activeApp.status}
 onChange={(e) => handleStatusDropdownChange(e.target.value)}
 className="border border-(--hairline) font-medium text-sm p-1.5 bg-white cursor-pointer"
 >
 {COLUMNS.map(col => (
 ...
 <div>
 <label className="block font-semibold text-xs text-gray-500 mb-1">Company Name</label>
 <input 
 className="w-full border border-(--hairline) p-2 font-bold outline-none focus:bg-yellow-50"
 ...
 <div>
 <label className="block font-semibold text-xs text-gray-500 mb-1">Role / Title</label>
 <input 
 className="w-full border border-(--hairline) p-2 font-bold outline-none focus:bg-yellow-50"
 ...
 <div>
 <label className="block font-semibold text-xs text-gray-500 mb-1">Salary Range</label>
 <input 
 className="w-full border border-(--hairline) p-2 font-bold outline-none focus:bg-yellow-50"
 ...
 <div>
 <label className="block font-semibold text-xs text-gray-500 mb-1">Job URL</label>
 <div className="flex gap-2">
 <input 
 type="url"
 className="w-full border border-(--hairline) p-2 font-bold outline-none focus:bg-yellow-50 text-sm"
 value={activeApp.url || ''}
 placeholder="https://..."
 ...
 target="_blank"
 rel="noopener noreferrer"
 className="border border-(--hairline) bg-white p-2.5 hover:bg-gray-100 shadow-sm active:shadow-none shrink-0"
 >
 ...
 <div>
 <label className="block font-semibold text-xs text-gray-500 mb-1">Application Custom Notes</label>
 <textarea 
 rows={6}
 className="w-full border border-(--hairline) p-3 font-bold outline-none focus:bg-yellow-50 text-sm"
 ...
 <h3 className="text-xl font-medium">Interview Checklist</h3>
 ...
 <input 
 required
 className="flex-1 border border-(--hairline) p-2 font-bold focus:bg-yellow-50 outline-none"
 placeholder="Add new preparation task..."
 value={newChecklistItem}
 onChange={(e) => setNewChecklistItem(e.target.value)}
 />
 <Button type="submit" variant="default" className="bg-(--accent-amber) border border-(--hairline) text-black px-4 shadow-sm">
 Add
 </Button>
 </form>
 ...
 <div className="text-center font-bold text-gray-500 py-8 border border-dashed border-(--hairline-soft)">
 ...
 <div 
 key={item.id}
 className={`flex items-center justify-between p-3 border border-(--hairline) shadow-sm hover:shadow-md transition-all ${
 ...
 className="w-5 h-5 border border-(--hairline) text-black rounded-xl cursor-pointer focus:ring-0"
 />
 <span>{item.text}</span>
 </label>
 <button 
 onClick={() => handleDeleteChecklistItem(item.id)}
 className="text-red-500 hover:text-red-700 bg-white border border-(--hairline) p-1 shrink-0"
 >
 <Trash2 className="w-3.5 h-3.5" />
 </button>
 </div>
 ...
 <div className="space-y-6">
 <div className="border-b border-(--hairline) pb-2">
 <h3 className="text-xl font-medium">Follow-up Reminders</h3>
 <p className="text-sm font-semibold text-gray-500">Log calendar events, recruiter replies, and application milestones.</p>
 </div>

 <form onSubmit={handleAddReminder} className="bg-gray-50 border border-(--hairline) p-4 space-y-3 shadow-sm">
 <div className="font-semibold text-sm">Schedule Follow-up:</div>
 <div className="flex flex-col md:flex-row gap-3">
 <input 
 required
 className="flex-1 border border-(--hairline) p-2 font-bold focus:bg-white outline-none text-sm bg-white"
 ...
 required
 type="date"
 className="border border-(--hairline) p-2 font-bold focus:bg-white outline-none text-sm bg-white"
 value={newReminderDate}
 onChange={(e) => setNewReminderDate(e.target.value)}
 />
 <Button type="submit" variant="default" className="bg-(--primary-active) border border-(--hairline) text-black px-4 shadow-sm shrink-0">
 Set Reminder
 </Button>
 </div>
 </form>
 ...
 <div 
 key={rem.id}
 className={`flex items-start md:items-center justify-between p-3 border border-(--hairline) shadow-sm transition-all ${
 ...
 className="w-5 h-5 border border-(--hairline) text-black rounded-xl cursor-pointer mt-0.5 shrink-0"
 />
 <div>
 <span className="font-extrabold text-sm block">{rem.text}</span>
 <span className={`inline-flex items-center gap-1 text-xs font-bold mt-1 px-1.5 py-0.5 border border-(--hairline) ${
 rem.done ? 'bg-gray-100 text-gray-400 border-gray-300' :
 isOverdue ? 'bg-red-500 text-white font-semibold animate-pulse' : 'bg-(--primary) text-black'
 }`}>
 ...
 className="text-red-500 hover:text-red-700 bg-white border border-(--hairline) p-1 shrink-0 ml-4"
 >
 <Trash2 className="w-3.5 h-3.5" />
 </button>
 </div>
 ...
 <div className="space-y-6">
 <div className="border-b border-(--hairline) pb-2">
 <h3 className="text-xl font-medium">Application Timeline Logs</h3>
 <p className="text-sm font-semibold text-gray-500">Detailed historical record of your application status progression.</p>
 </div>

 <div className="relative pl-8 border-l-[3px] border-l-(--primary) ml-4 space-y-8 py-2">
 ...
 <div className="absolute -left-[42px] top-1.5 w-6 h-6 border border-(--hairline) bg-white shadow-sm flex items-center justify-center shrink-0">
 ...
 {/* Modal Footer */}
 <div className="p-4 border-t border-(--hairline) bg-gray-50 flex flex-wrap justify-between items-center gap-4">
 <Button 
 variant="white" 
 onClick={(e) => handleDelete(activeApp.id, e)} 
 className="bg-red-50 text-red-600 border border-red-500 font-extrabold shadow-sm hover:bg-red-100"
 >
 ...
 className="font-bold border border-(--hairline)"
 >
 Close
 </Button>
 </div>
