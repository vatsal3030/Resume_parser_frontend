...
function LoginForm() {
 ...
 <CardDescription>Sign in to your Elevara account</CardDescription>
 </CardHeader>
 ...
 <p className="text-xs font-medium text-gray-500">Quick Login — Saved Accounts</p>
 {savedAccounts.map((acc) => (
 <div key={acc.id} className="flex items-center gap-3 p-3 border border-(--hairline) bg-(--canvas) hover:bg-(--primary)/20 cursor-pointer shadow-sm transition-all group"
 onClick={() => {
 setEmail(acc.email);
 setSuccess(`Logging in as ${acc.name || acc.email}. Enter your password.`);
 }}
 >
 <div className="w-10 h-10 bg-white border border-(--hairline) flex items-center justify-center overflow-hidden shrink-0">
 {acc.avatarUrl ? (
 /* eslint-disable-next-line @next/next/no-img-element */
 <img src={acc.avatarUrl} alt="" className="w-full h-full object-cover" />
 ) : (
 <span className="text-lg font-semibold">{(acc.name || acc.email)?.[0]?.toUpperCase()}</span>
 )}
 </div>
 <div className="flex-1 min-w-0">
 <p className="font-semibold text-sm truncate">{acc.name || 'User'}</p>
 ...
 <span className="shrink-0 px-4 text-xs font-medium text-(--muted) bg-white mx-2">OR</span>
 <div className="grow border-t border-(--hairline)"></div>
 </div>
 ...
 <p className="text-center text-sm text-(--muted) pb-2 pt-4">
 Don&apos;t have an account? <Link href="/register" className="text-(--primary) font-medium hover:text-(--primary-active) transition-colors">Register</Link>
 </p>
 </CardContent>
 </Card>
 );
