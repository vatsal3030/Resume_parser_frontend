...
export default function RegisterPage() {
 ...
 <Card>
 <CardHeader className="bg-(--accent-teal) border-b border-(--hairline)">
 ...
 {error && (
 <div className="p-3 border border-(--hairline) bg-red-400 text-(--ink) font-bold shadow-sm">
 {error}
 </div>
 )}
 ...
 <span className="shrink-0 px-4 font-bold text-sm bg-(--primary) border border-(--hairline) rounded-full mx-2 shadow-sm">OR</span>
 <div className="grow border-t border-(--hairline)"></div>
 </div>
 ...
 Already have an account? <Link href="/login" className="bg-(--primary-active) px-2 py-1 border border-(--hairline) hover:bg-(--primary) transition-colors shadow-sm">Sign In</Link>
 </p>
 </>
 ) : (
 ...
 className={`relative w-full aspect-square border transition-all ${selectedAvatar === url ? 'border-(--hairline) bg-(--primary) shadow-sm scale-105' : 'border-transparent bg-(--canvas) hover:border-(--hairline) hover:shadow-sm'}`}
 >
 ...
 <Button 
 variant="default" 
 className="flex-1 bg-(--accent-amber)" 
