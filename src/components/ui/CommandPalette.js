...
export function CommandPalette({ isOpen, setIsOpen }) {
 ...
 <div 
 className="relative w-full max-w-2xl bg-(--surface-card) rounded-2xl border border-(--hairline) shadow-2xl overflow-hidden"
 onKeyDown={handlePaletteKeyDown}
 >
 {/* Search Input */}
 <div className="flex items-center px-5 py-4 border-b border-(--hairline-soft)">
 <Search className="w-5 h-5 text-(--muted-soft) mr-3" />
 <input
 ref={inputRef}
 type="text"
 className="flex-1 text-base bg-transparent outline-none placeholder:text-(--muted-soft) text-(--ink)"
 ...
 />
 <button onClick={() => setIsOpen(false)} className="p-1.5 rounded-lg text-(--muted-soft) hover:text-(--ink) hover:bg-(--surface-soft) transition-colors">
 <X className="w-4 h-4" />
 </button>
 </div>
 ...
 >
 <action.icon className={`w-4 h-4 mr-3 ${isSelected ? 'text-(--primary)' : 'text-(--muted-soft)'}`} />
 <div className="flex-1 flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3">
 <span className="font-medium text-sm">{action.name}</span>
 <span className="text-[10px] font-medium text-(--muted-soft) bg-(--surface-soft) px-1.5 py-0.5 rounded">
 {action.group}
 </span>
 </div>
 {isSelected && <ArrowRight className="w-4 h-4 text-(--muted-soft)" />}
 </button>
 );
 })}
 </div>
