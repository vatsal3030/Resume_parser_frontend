...
const TOAST_STYLES = {
  success: 'bg-(--surface-card)/95 backdrop-blur-xl border border-(--hairline) border-l-4 border-l-emerald-500 text-(--ink)',
  error: 'bg-(--surface-card)/95 backdrop-blur-xl border border-(--hairline) border-l-4 border-l-red-500 text-(--ink)',
  warning: 'bg-(--surface-card)/95 backdrop-blur-xl border border-(--hairline) border-l-4 border-l-amber-500 text-(--ink)',
  info: 'bg-(--surface-card)/95 backdrop-blur-xl border border-(--hairline) border-l-4 border-l-sky-500 text-(--ink)',
};
...
export function ToastProvider({ children }) {
 ...
 rounded-xl border border-(--hairline) shadow-lg
 ${TOAST_STYLES[t.type]}
 ${t.exiting ? 'animate-toast-exit' : 'animate-toast-enter'}
 `}
 role="alert"
 >
 <Icon className={`w-5 h-5 mt-0.5 shrink-0 ${TOAST_ICON_STYLES[t.type] || ''}`} strokeWidth={2} />
 <div className="flex-1 min-w-0">
 {t.title && (
 <p className="font-medium text-sm leading-tight text-(--ink)">{t.title}</p>
 )}
 {t.message && (
 <p className="text-sm mt-0.5 text-(--muted) leading-snug">{t.message}</p>
 )}
 {t.action && (
 <button 
 onClick={() => {
 ...
 className="mt-2 text-xs font-medium text-(--primary) hover:text-(--primary-active) transition-colors"
 >
 {t.action.label}
 </button>
 )}
 ...
 className="shrink-0 p-1 rounded-md text-(--muted-soft) hover:text-(--ink) hover:bg-(--surface-soft) transition-colors"
 aria-label="Close"
 >
